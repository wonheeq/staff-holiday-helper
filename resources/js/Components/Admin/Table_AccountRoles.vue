...
export default {
    ...
    data: function() {
        ...
        return {
            columns: [
                {
                label: 'AccountRole ID',
                field: 'accountRoleId',
                },
                {
                label: 'ID of staff member filling the role',
                field: 'accountNo',
                },
                {
                label: 'Role Id',
    ...
    created() {
        //console.warn("/api/allAccountRoles/" + this.user)
        axios.get("/api/allAccountRoles/" + this.user)
        .then((response) => {
    ...
    methods: {  
        ...
        deleteClicked: function(rowId) {
            ...
            Swal.fire({
                icon: 'warning',
                title: 'Delete \'' + rowId + '\'?',
                text: 'This will not only remove the account role from the database, but also all nominations associated in any way with the account role.',
                showDenyButton: true,
        ...
        deleteEntry: function(rowId) {
            ...
            // Removing AccountRole from DB
            axios.post("/api/dropEntry/" + this.user, data)
            .then((response) => {
                if (response.status == 200) {   
                    Swal.fire({
                        icon: "success",
                        title: 'Successfully deleted account role.'
                    });
        ...
        <div class="laptop:mx-4 laptop:mt-4 4k:mt-8">
            <div remove-tailwind-bg>
                <VueGoodTable 
                    ...
                    :search-options="{
                        enabled: true,
                        placeholder: 'Search Roles of Staff-Members',
                    }"
                    :pagination-options="{
                        enabled: true,
                        //mode: 'pages',
                    ...
                    <template #table-row="props">
                        <span v-if="props.column.field == 'delete'">
                            <button type="button" class="4k:w-10 4k:h-10" v-on:click="deleteClicked(props.row.accountRoleId)">
